...
How It Works
The ESP32 scans an RFID tag using the MFRC522 module.


It sends the scanned RFID data to the PHP server.


The PHP script checks if the RFID exists in the database.


If registered, the server automatically toggles its status and logs the activity.


The result (1, 0, or “RFID NOT FOUND”) is returned to the ESP32.


The ESP32 publishes this result to the MQTT broker.


ESP32 #2 receives the status and updates the relay and LED accordingly.


The web interface refreshes automatically to show the updated log and current RFID statuses.
